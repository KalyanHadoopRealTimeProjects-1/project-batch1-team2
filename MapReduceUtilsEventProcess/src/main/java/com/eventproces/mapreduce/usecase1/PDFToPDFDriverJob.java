...
import com.eventproces.mapreduce.usecase1.TextFileToTextDriverJob.TextToTextMapper;
import com.eventproces.mapreduce.usecase1.TextFileToTextDriverJob.TextToTextReducer;
import com.eventproces.mapreduce.utils.EventConstants;
import com.eventproces.mapreduce.utils.EventProcessLog;
import com.eventproces.mapreduce.utils.OrenCustomPDFInputFormat;
import com.eventproces.mapreduce.utils.OrenCustomPDFOutputFormat;

public class PDFToPDFDriverJob implements Tool{
	...
	public int run(String[] args) throws Exception {
		...
		job.setInputFormatClass(OrenCustomPDFInputFormat.class);
		//LazyOutputFormat.setOutputFormatClass(job, KalyanPdfOutputFormat.class);
		
		MultipleOutputs.addNamedOutput(job,"SUCCESS", OrenCustomPDFOutputFormat.class,Text.class,Text.class);
		MultipleOutputs.addNamedOutput(job,"ERROR", OrenCustomPDFOutputFormat.class,Text.class,Text.class);
		MultipleOutputs.addNamedOutput(job,"NA", OrenCustomPDFOutputFormat.class,Text.class,Text.class);
	...
	public static class PDFToPDFMapper	extends	org.apache.hadoop.mapreduce.Mapper<LongWritable, Text, Text, EventProcessLog> {
	...
	public static class PDFToPDFReducer extends Reducer<Text, EventProcessLog, Text, Text> {
...
*/
				 ...
				 if ("SUCCESS".equalsIgnoreCase(value.getStatus())) {
						mos.write("SUCCESS",key ,new Text(value.toString()));
				} else if ("ERROR".equalsIgnoreCase(value.getStatus())) {
					mos.write("ERROR",key ,new Text(value.toString()));
				} else {
					mos.write("NA",key ,new Text(value.toString()));
				}
...
public static void main(String[] args) {
	try{
	int textStatus = ToolRunner.run( new Configuration(),	new PDFToPDFDriverJob(), args);
